...
Snake** alloc_snake(){
    Snake** snakes=(Snake**)malloc(sizeof(Snake*)*MAX_COUNT);
    if (snakes==NULL) {
...
void Draw_Snake(Snake** snakes, size_t count){
    for (size_t i=0; i<count;i++){
    DrawRectangleV(snakes[i]->position, (Vector2){.x=SNAKE_SIZE,.y=SNAKE_SIZE}, GREEN);
    }

}
//this function is responsable of the movement of the snake
DIRECTION* Move_Snakes(Snake** snakes,DIRECTION* direction,Vector2* next_pos,size_t count){
...
void initialize_snake(Snake**snakes){
...
int main(void){
    ...
    while(!WindowShouldClose()){
            ...
            food_is_eaten=CheckCollisionPointCircle(
                (Vector2){.x=(snakes[0]->position.x)/2.5,.y=(snakes[0]->position.y)/2.5},
                (Vector2){.x=food_pos.x/2.5,.y=food_pos.y/2.5},
                 FOOD_SIZE) ;
            for(size_t i=4;i<count;i++){
            Rectangle rec = {
                .x=snakes[i]->position.x-SNAKE_SIZE,
                .y=snakes[i]->position.y-SNAKE_SIZE,
                .width=SNAKE_SIZE,
                .height=SNAKE_SIZE
            };
            bool snake_collided_with_itself=CheckCollisionPointRec(snakes[0]->position, rec);
        ...
        if (food_is_eaten){
            ...
            count=count+1;  //increase the size of the snake after the food_is_eaten
        ...
        if (!snake_is_alive) {
            ...
            DrawText("PRESS ENTER to START OVER",GetScreenWidth()/8.0, 180,40 , BLUE);
            if(IsKeyPressed(KEY_ENTER)){
                //by pressing enter th game start over
                free_snake(snakes);       // deallocating the memory
                count=COUNT;              // setting the count to 4
                snakes=alloc_snake();     // allocting the memory again because it was freed
                initialize_snake(snakes);
                snake_is_alive=true;
                food_is_eaten=false;
            }
